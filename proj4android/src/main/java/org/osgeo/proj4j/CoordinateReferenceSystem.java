 ...
 * <li>a {@link Datum} provides the mapping from the ellipsoid to actual
 * locations on the earth
 * <li>a {@link Projection} method maps the ellpsoidal surface to a planar
 * space. (The projection method may be null in the case of geodetic coordinate
 * systems).
 * <li>a {@link Unit} indicates how the ordinate values of coordinates are
 * interpreted
 * </ul>
 * 
 * @author Martin Davis
...
public class CoordinateReferenceSystem {
	// allows specifying transformations which convert to/from Geographic
	// coordinates on the same datum
	public static final CoordinateReferenceSystem CS_GEO = new CoordinateReferenceSystem(
			"CS_GEO", null, null, null);

	// TODO: add metadata like authority, id, name, parameter string, datum,
	// ellipsoid, datum shift parameters
	...
	public CoordinateReferenceSystem(String name, String[] params, Datum datum,
			Projection proj) {
		this.name = name;
		this.params = params;
		this.datum = datum;
	...
	public CoordinateReferenceSystem createGeographic() {
		...
		geoProj.initialize();
		return new CoordinateReferenceSystem("GEO-" + datum.getCode(), null,
				datum, geoProj);
	}
