...
/**
 * This class is essentially obsolete. Use {@link CoordinateSystemFactory}
 * instead.
 * 
 * @author Martin Davis
 * 
...
public class ProjectionFactory {
	...
	public static Projection fromPROJ4Specification(String[] args) {
		...
		if (s != null)
			...
			if (s != null) {
				ellipsoid = registry.getEllipsoid(s);
				if (ellipsoid == null)
						...
						if (s != null && Boolean.getBoolean(s)) {
							...
							if (s != null && Boolean.getBoolean(s)) {
								...
								if (s != null) {
									...
									if (Math.abs(tmp) > ProjectionMath.HALFPI)
		...
		if (s != null)
			throw new UnsupportedOperationException("towgs84");

		s = (String) params.get("lonc");
		if (s != null)
			throw new UnsupportedOperationException("lonc");

		s = (String) params.get("south");
		if (s != null)
			throw new UnsupportedOperationException("south");

		// TODO: refine this test. +datum is supported, but not for datum
		// transformation at this point
		/*
		 * s = (String)params.get( "datum" ); if ( s != null ) throw new
		 * UnsupportedOperationException("datum");
		 */
	...
	public static Projection readProjectionFile(String file, String name)
			throws IOException {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(
				ProjectionFactory.class.getResourceAsStream("/nad/" + file)));
		...
		while (t.ttype == '<') {
			...
			while (t.ttype != '<') {
				if (t.ttype == '+')
					t.nextToken();
				if (t.ttype != StreamTokenizer.TT_WORD)
					throw new IOException(t.lineno()
							+ ": Word expected after '+'");
				String key = t.sval;
				t.nextToken();
				if (t.ttype == '=') {
	...
	public static Projection getNamedPROJ4CoordinateSystem(String name) {
		...
		try {
			int p = name.indexOf(':');
			if (p >= 0)
				return readProjectionFile(name.substring(0, p),
						name.substring(p + 1));
	...
	// TODO: create new cmd-line interface similar to PROJ4 ones
...
//						}
//					} catch (IOException e) {
//						System.out.println("IOException: " + args[i] + ": "
//								+ e.getMessage());
//					}
//				}
//			}
