...
public class SwdyxCgAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (BussUtil.isEmperty(list.get(position).get("PAY").toString())) {
			holder.tv4.setText(list.get(position).get("PAY").toString());
		} else {
			holder.tv4.setText("");
		}
	...
	protected void showCheckDailog(HashMap<String, String> map) {
		...
		if (BussUtil.isEmperty(map.get("NAME").toString())) {
			swdyx_cg_cgmc.setText(map.get("NAME").toString());
		} else {
			swdyx_cg_cgmc.setText("");
		}
		// 人均消费
		if (BussUtil.isEmperty(map.get("PAY").toString())) {
			swdyx_cg_pay.setText(map.get("PAY").toString());
		} else {
			swdyx_cg_pay.setText("");
		}
		// 经度
		if (BussUtil.isEmperty(map.get("X").toString())) {
			swdyx_cg_jd.setText(map.get("X").toString());
		} else {
			swdyx_cg_jd.setText("");
		}
		// 纬度
		if (BussUtil.isEmperty(map.get("Y").toString())) {
			swdyx_cg_wd.setText(map.get("Y").toString());
		} else {
			swdyx_cg_wd.setText("");
		}
		// 地址
		if (BussUtil.isEmperty(map.get("ADDRESS").toString())) {
			swdyx_cg_dz.setText(map.get("ADDRESS").toString());
		} else {
			swdyx_cg_dz.setText("");
		}
	...
	protected void showEditDailog(final HashMap<String, String> map) {
		...
		Button cancle = (Button) dialog
				.findViewById(R.id.swdyx_cg_cancle);
