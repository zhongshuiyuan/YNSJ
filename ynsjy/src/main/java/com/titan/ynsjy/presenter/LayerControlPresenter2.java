...
import com.titan.ynsjy.adapter.TckzListViewAdapter;
...
public class LayerControlPresenter2 implements TckzListViewAdapter.addLayerInMapview{
...
//        });
    ...
    private List<TitanLayer> initLayerData(List<File> groups,List<Map<String, List<File>>> childs){
        int i = 0;
        //一级目录
    ...
    @Override
    public void addLayer(boolean flag,String path,String gname,String cname,String tableName) {
        changeCBoxStatus(flag, path, gname, cname,tableName);
    }
    ...
    /** checkbox状态变化 */
    public void changeCBoxStatus(boolean flag, final String path,final String gpname, final String childName,String tableName) {
        if(flag){
            ...
            loadGeodatabase(path,flag2,gpname,childName,tableName);
        }else{
//            controlView.getLayerCheckBox().put(path, false);
            for (int j = 0; j < controlView.getLayerNameList().size();j++) {
                ...
                tname = controlView.getLayerNameList().get(j).getTname();
                if(gpname.equals(gname)&&childName.equals(cname)&&tname.equals(tableName)){
    ...
    public void loadGeodatabase(String path, boolean flag, String gname,String cname,String tableName) {
        boolean ff = false;
            try {
            ...
            for (GeodatabaseFeatureTable gdbFeatureTable : list) {
                if (!tableName.equals(gdbFeatureTable.getTableName())){
                    continue;
                }
                if (gdbFeatureTable.hasGeometry()) {
                    ...
                    if(controlView.getTitleLayer() != null && controlView.getTitleLayer().isInitialized()){
                        ...
                        Log.e("tag","sp1"+sp1+",sp2:"+sp2+",name"+gdbFeatureTable.getTableName());
                        if(!sp1.equals(sp2)){
                    ...
                    setMyLayer(gname,cname,tableName,path,flag,layer,gdbFeatureTable);
...
//            controlView.getLayerCheckBox().put(path, true);
    ...
    //设置MyLayer的相关信息
    private void setMyLayer(String gname,String cname,String tableName,String path,boolean flag,FeatureLayer layer,GeodatabaseFeatureTable featureTable){
