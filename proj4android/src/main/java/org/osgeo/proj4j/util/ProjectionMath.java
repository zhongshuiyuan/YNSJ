...
public class ProjectionMath {
	...
	public static double sphericalAzimuth(double lat0, double lon0, double lat,
		...
		return Math.atan2(coslat * Math.sin(diff),
				(Math.cos(lat0) * Math.sin(lat) - Math.sin(lat0) * coslat
						* Math.cos(diff)));
	}
	 ...
	 * 
	 * public final static int DONT_INTERSECT = 0; public final static int
	 * DO_INTERSECT = 1; public final static int COLLINEAR = 2;
	 * 
	 * public static int intersectSegments(Point2D.Double aStart, Point2D.Double
	 * aEnd, Point2D.Double bStart, Point2D.Double bEnd, Point2D.Double p) {
	 * double a1, a2, b1, b2, c1, c2; double r1, r2, r3, r4; double denom,
	 * offset, num;
	 * 
	 * a1 = aEnd.y-aStart.y; b1 = aStart.x-aEnd.x; c1 = aEnd.x*aStart.y -
	 * aStart.x*aEnd.y; r3 = a1*bStart.x + b1*bStart.y + c1; r4 = a1*bEnd.x +
	 * b1*bEnd.y + c1;
	 * 
	 * if (r3 != 0 && r4 != 0 && sameSigns(r3, r4)) return DONT_INTERSECT;
	 * 
	 * a2 = bEnd.y-bStart.y; b2 = bStart.x-bEnd.x; c2 =
	 * bEnd.x*bStart.y-bStart.x*bEnd.y; r1 = a2*aStart.x + b2*aStart.y + c2; r2
	 * = a2*aEnd.x + b2*aEnd.y + c2;
	 * 
	 * if (r1 != 0 && r2 != 0 && sameSigns(r1, r2)) return DONT_INTERSECT;
	 * 
	 * denom = a1*b2 - a2*b1; if (denom == 0) return COLLINEAR;
	 * 
	 * offset = denom < 0 ? -denom/2 : denom/2;
	 * 
	 * num = b1*c2 - b2*c1; p.x = (num < 0 ? num-offset : num+offset) / denom;
	 * 
	 * num = a2*c1 - a1*c2; p.y = (num < 0 ? num-offset : num+offset) / denom;
	 * 
	 * return DO_INTERSECT; }
	 * 
	 * /* public static double dot(Point2D.Double a, Point2D.Double b) { return
	 * a.x*b.x + a.y*b.y; }
	 * 
	 * public static Point2D.Double perpendicular(Point2D.Double a) { return new
	 * Point2D.Double(-a.y, a.x); }
	 * 
	 ...
	 * { return new Point2D.Double(a.x*b.x, a.y*b.y); }
	 * 
	 * public static double cross(Point2D.Double a, Point2D.Double b) { return
	 * a.x*b.y - b.x*a.y; }
	 * 
	 * public static void normalize(Point2D.Double a) { double d = distance(a.x,
	 * a.y); a.x /= d; a.y /= d; }
	 * 
	 * public static void negate(Point2D.Double a) { a.x = -a.x; a.y = -a.y; }
	 */
	...
	public static double tsfn(double phi, double sinphi, double e) {
		sinphi *= e;
		return (Math.tan(.5 * (ProjectionMath.HALFPI - phi)) / Math.pow(
				(1. - sinphi) / (1. + sinphi), .5 * e));
	}
	...
	public static double phi2(double ts, double e) {
		...
		if (i <= 0)
			throw new ConvergenceFailureException(
					"Computation of phi2 failed to converage after " + N_ITER
							+ " iterations");
		return phi;
	}
