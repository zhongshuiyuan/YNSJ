 ...
 * {@link CoordinateReferenceSystem} into a different one, using reprojection
 * and datum conversion as required.
 * <p>
 * Computing the transform involves the following steps:
 * <ul>
 * <li>If the source coordinate is in a projected coordinate system, it is
 * inverse-projected into a geographic coordinate system based on the source
 * datum
 * <li>If the source and target {@link Datum}s are different, the source
 * geographic coordinate is converted from the source to the target datum as
 * accurately as possible
 * <li>If the target coordinate system is a projected coordinate system, the
 * converted geographic coordinate is projected into a projected coordinate.
 * </ul>
 ...
 * </pre>
 * 
 * <tt>BasicCoordinateTransform</tt> objects are stateful, and thus are not
 * thread-safe. However, they may be reused any number of times within a single
 * thread.
 * <p>
 * Information about the transformation procedure is pre-computed and cached in
 * this object for efficient computation.
 * 
 * @author Martin Davis
...
public class BasicCoordinateTransform implements CoordinateTransform {
	...
	public BasicCoordinateTransform(CoordinateReferenceSystem srcCRS,
		...
		// compute strategy for transformation at initialization time, to make
		...
		if (doDatumTransform) {
			...
			boolean isEllipsoidEqual = srcCRS.getDatum().getEllipsoid()
					.isEqual(tgtCRS.getDatum().getEllipsoid());
			if (!isEllipsoidEqual)
			...
			if (transformViaGeocentric) {
				srcGeoConv = new GeocentricConverter(srcCRS.getDatum()
						.getEllipsoid());
				tgtGeoConv = new GeocentricConverter(tgtCRS.getDatum()
						.getEllipsoid());
			}
		}
	}
	 ...
	 * 
	 * @param src
	 *            the input coordinate to be transformed
	 * @param tgt
	...
	public ProjCoordinate transform(ProjCoordinate src, ProjCoordinate tgt)
			throws Proj4jException {
		// NOTE: this method may be called many times, so needs to be as
	...
	/**
	 * 
	 * Input: long/lat/z coordinates in radians in the source datum Output:
	 * long/lat/z coordinates in radians in the target datum
	 * 
	 * @param pt
